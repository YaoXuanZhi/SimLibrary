...
* @mark 要想查询某个键值，则必须要事先知道其子键路径和键名
*		然后根据其键名来查询对应的注册表信息。根据上述思路，
*		定义了下面这个数据结构来存放查询键值的条件。
* @not  注意，在这里面Ctx的原意为context
*/
class RegKeyCtx 
{
public:
...
/** @brief: 定义键值数据结构
* @mark 键值有多种类型，如REG_SZ、REG_DWORD，并且在查询注册表的时候，
...
public:
	...
	*/
	...
	/** @brief: 初始化字符串键值类型
	* @mark  dwValueType可能为REG_SZ、REG_EXPAND_SZ、REG_MULTI_SZ
	*/
	RegValueCtx(const TCHAR* szBuffer, unsigned long dwValueType = REG_SZ);

	/** @brief: 初始化REG_QWORD键值类型
	* @mark  因为unsigned long lon和unsigned long兼容，所以需要特意
...
public:
	...
	* @mark 注意在执行此函数之前要执行SetValueType()函数
	*/
	...
	* @return  无
	* @mark  此函数和SetBufferByHexStr()是一对
	*/
	...
	* @mark  此函数和GetHexStrForBuffer()是一对
	*/
...
* @return  无
* @mark  最后一个参数一般都是将类指针，或者是其它可变数值传进来的
*/
...
namespace IRegEditor 
{
	...
	* @return  如果返回为ERROR_SUCCESS则成功，否则需要查询异常代码
	* @mark  MSDN原文，Deletes a subkey and its values. Note that key names are not case sensitive.
	...
	* @return  如果返回为true，则获取成功，否则获取失败
	* @mark  之所以以传入的KEYVALUESTRUCT&作为键值，这是因为当键值为REG_BINARY的时候，
	...
	* @return  如果返回为true，成功删除指定键值，否则失败
	* @mark
	*/
	...
	* @return  返回为truef，则说明起码找到了其中一个，否则将代表查询失败
	* @mark  mapValues的第一个数值将作为查询的键名，第二个数值则是存放查询所得的结果
	*/
	...
	* @return  返回为true，则说明起码找到了其中一个，否则将代表查询失败
	* @mark  mapValues的第一个数值将作为查询的键名，第二个数值则是存放查询所得的结果
	*/
	...
	* @return  返回为false，则说明起码有一个赋值失败，或者是当前待赋值的键值信息为空
	* @mark  mapValues的第一个数值将作为查询的键名，第二个数值则是存放查询所得的结果
	*/
